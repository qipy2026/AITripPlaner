...
/** 默认行程日：下个月 1 日（开始、结束同一天，便于 1 日游） */
const defaultTripDay = dayjs().add(1, 'month').startOf('month')

type HomeFormState = Omit<TripFormData, 'start_date' | 'end_date'> & {
...
const formData = reactive<HomeFormState>({
  city: '北京',
  start_date: defaultTripDay,
  end_date: defaultTripDay,
  travel_days: 1,
  transportation: '公共交通',
  accommodation: '经济型酒店',
  preferences: [],
